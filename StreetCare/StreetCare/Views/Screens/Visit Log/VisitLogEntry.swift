...
struct VisitLogEntry: View {
    ...
    var body: some View {
        NavigationStack {
            VStack(spacing: 10) {
                ...
                case 1:
                    InputTileDetails(
                        ...
                        nextAction: {
                            visitLog.whenVisit = rawDate
                            visitLog.whenVisitEnd = rawEndDate ?? rawDate
                            visitLog.timezone = selectedTimeZone
                            let cityName = selectedTimeZone.split(separator: "/").last?.replacingOccurrences(of: "_", with: " ") ?? selectedTimeZone
                            visitLog.city = String(cityName)
                            questionNumber += 1
                        },
                        skipAction: { questionNumber += 1 },
                ...
                case 8:
                    InputTileIndividualInteraction(
                        // Pass the main visitLog object so the tile can access 'timezone'
                        visitLog: visitLog,
                        log: currentInteractionLog,
                        questionTitle: "",
                        questionNumber: 1,
                ...
                case 11:
                    InputTileFollowUpDate(
                        ...
                        convertedDate: $currentInteractionLog.helpRequestFollowUpTimestamp,
                        additionalDetails: $currentInteractionLog.helpRequestAdditionalDetails,
                        selectedTimeZone: visitLog.timezone,
                        nextAction: {
                            if didCommitOnThisPass {
                                questionNumber = 12
                                return
                            ...
                            didCommitOnThisPass = true
                            let title = isCreatingNewInteraction ? "Individual Interaction \(individualInteractions.count + 1)" : "Individual Interaction \( (editingIndex ?? 0) + 1)"
                            let newItem = IndividualInteractionItem(
                                title: title,
                                firstName: currentInteractionLog.recipientFirstName,
                                lastName: currentInteractionLog.recipientLastName,
                            ...
                            if isCreatingNewInteraction {
                                individualInteractions.append(newItem)
                            } else if let idx = editingIndex, individualInteractions.indices.contains(idx) {
                                individualInteractions[idx] = newItem
                            }
                            currentInteractionLog = VisitLog(id: UUID().uuidString)
                            questionNumber = 12
                ...
                case 12:
                    InputTileIndividualInteractionsSummary(
                        ...
                        deleteAction: { _, index in
                            if individualInteractions.indices.contains(index) {
                                individualInteractions.remove(at: index)
                                renumberIndividualInteractionTitles()
                            }
                        },
                ...
                case 13:
                    InputTileDate(
                        ...
                        // FIX: We pass the actual string value.
                        // We use 'selectedTimeZone' (the local state variable that was set in Case 1)
    ...
    // --- Functions defined outside the body ---
