...
struct OutreachFormView: View {
    @Binding var isPresented: Bool // Binding to dismiss ICanHelpView
    ...
    func saveToFirestore() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
            ...
            alertMessage = "Event saved successfully!"
            showAlert = true
            isPresented = false // Close both views after saving
        }
    }
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 18) {
                ...
                VStack(spacing: 20) {
                    ...
                    NavLinkButton(title: NSLocalizedString("discardButtonTitle", comment: ""), width: 300, secondaryButton: true)
                        .onTapGesture {
                            isPresented = false // Close both views on discard
                        }
                }
        ...
        .toolbar {
            ToolbarItem(placement: .navigationBarLeading) {
                Button(action: {
                    isPresented = false // Close both views on back
                }) {
                    Image(systemName: "chevron.left")
